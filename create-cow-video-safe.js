...
async function createCowVideo() {
  ...
  // Using only safe transitions that we know work
  const safeTransitions = ['fade', 'dissolve'];
  ...
  const videoClips = cowVideos.map((filename, index) => {
    ...
    const transition = safeTransitions[index % safeTransitions.length];
  ...
  const videoConfig = {
    ...
    clips: [
      ...
      // All the cow video clips with text overlays and safe transitions
      ...videoClips
    ]
  };
  ...
  console.log('Using safe transitions to ensure completion...\n');
